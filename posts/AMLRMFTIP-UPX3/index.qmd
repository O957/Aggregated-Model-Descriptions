---
title: "A Multilevel Regression Model For Tornadoes, In Python"
description: "_This model is the first in a series of statistical models for forecasting tornadoes, as part of a question featured on Metaculus._"
date: "2024-09-01"
date-modified: "2024-09-01"
bibliography: references.bib
link-citations: true
categories: [modeling, data-science, mathematics]
csl: "../../assets/csls/vodohospodarske-technicko-ekonomicke-informace-en_MODIFIED.csl"
author:
  - name: O957
    url: https://github.com/O957
...
The tornado count $T_{ijk}$ for state $i$, month $j$, and year $k$ can be modelled via $T_{ijk} \sim \text{Poisson}(Y_{ijk})$, where $\log (Y_{ijk}) = \alpha_{\text{state}[i]} + \gamma_{\text{month}[j]} + \delta_{\text{year}[k]}$, i.e. $Y_{ijk}$ is [log-linear](https://en.wikipedia.org/wiki/Log-linear_model).
